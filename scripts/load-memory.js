...
// Uses a stable hook-runner.js that dynamically finds the latest plugin version,
// so settings.json paths don't break on plugin updates.
const HOOK_RUNNER_CODE = `// Auto-generated by memory-keeper plugin - DO NOT EDIT
// Finds latest memory-keeper plugin version and delegates to its scripts
// v2: reads stdin first, sets PROJECT_DIR from hookData.cwd for session isolation
const fs = require('fs');
const path = require('path');
const os = require('os');
const cacheBase = path.join(os.homedir(), '.claude', 'plugins', 'cache', 'memory-keeper-marketplace', 'memory-keeper');
if (!fs.existsSync(cacheBase)) process.exit(0);
const versions = fs.readdirSync(cacheBase).filter(d => /^\\d+\\.\\d+\\.\\d+$/.test(d));
...
const scriptPath = path.join(cacheBase, latest, 'scripts', scriptName);
if (!fs.existsSync(scriptPath)) process.exit(0);
process.argv = ['node', scriptPath, ...scriptArgs];
// Read stdin, extract cwd for PROJECT_DIR, then delegate to child script
let _data = '';
let _loaded = false;
function _run() {
  if (_loaded) return;
  _loaded = true;
  let hookData = {};
  try { hookData = _data.trim() ? JSON.parse(_data.trim()) : {}; } catch {}
  if (hookData.cwd) process.env.PROJECT_DIR = hookData.cwd;
  process.env.HOOK_DATA = _data.trim() || '{}';
  require(scriptPath);
}
process.stdin.setEncoding('utf8');
process.stdin.on('data', (c) => { _data += c; });
process.stdin.on('end', _run);
process.stdin.on('error', _run);
...
function ensureGlobalHooks() {
  try {
    const os = require('os');
    const runnerDir = path.join(os.homedir(), '.claude', 'memory-keeper');
    const runnerPath = path.join(runnerDir, 'hook-runner.js');
    const settingsPath = path.join(os.homedir(), '.claude', 'settings.json');
    const runnerPathFwd = runnerPath.replace(/\\/g, '/');

    // Ensure hook-runner.js exists and is up-to-date
    if (!fs.existsSync(runnerDir)) fs.mkdirSync(runnerDir, { recursive: true });
    const currentRunner = fs.existsSync(runnerPath) ? fs.readFileSync(runnerPath, 'utf8') : '';
    if (currentRunner !== HOOK_RUNNER_CODE) {
      fs.writeFileSync(runnerPath, HOOK_RUNNER_CODE);
    }

    const counterCmd = `node "${runnerPathFwd}" counter.js check`;
    const injectCmd = `node "${runnerPathFwd}" inject-rules.js`;
    ...
    if (modified) {
      ...
      console.error('[MEMORY_KEEPER] Registered hooks in ~/.claude/settings.json (workaround for plugin hook bug).');
    }
  } catch (e) {
...
readStdinAsync().then((stdinData) => {
  ...
  if (stdinData.cwd && !process.env.PROJECT_DIR) {
    process.env.PROJECT_DIR = stdinData.cwd;
  }
  try {
