...
- **Verification standard:** "File contains X" is NEVER valid verification. Always predict observable behavior. Success criteria must describe what happens, not what text exists in a file. **Runtime verification is mandatory** — verify that changes actually work when deployed/applied in practice: trigger → path through system → conditions → intended result. "Can verify but didn't" is a violation.
