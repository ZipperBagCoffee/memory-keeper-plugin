...
2. **Check processing lock** (race condition prevention):
   Use the Read tool to read `{PROJECT_DIR}/.claude/memory/delta_processing.lock`.
...
3. **Call delta-processor background agent**:
   ...
   INDEX_PATH: {PROJECT_DIR}/.claude/memory/memory-index.json
   TZ_OFFSET: {TZ_OFFSET}
   ```
...
**Fallback Step 4**: Append summary to memory.md:
   1. Use the Read tool to read `{PROJECT_DIR}/.claude/memory/memory.md` to get existing content.
   ...
   3. Use the Write tool to write `{PROJECT_DIR}/.claude/memory/memory.md` with: `{existing content}\n\n## {utc_timestamp} (local {local_timestamp})\n{summary}\n`
   4. Use the Read tool to read `{PROJECT_DIR}/.claude/memory/memory-index.json`, parse the JSON.
   5. Set `memoryAppendedInThisRun` to `true` in the JSON object.
   6. Use the Write tool to write `{PROJECT_DIR}/.claude/memory/memory-index.json` back (preserve ALL other fields, 2-space indentation).

**Fallback Step 5**: Update timestamp marker:
   1. Use the Read tool to read `{PROJECT_DIR}/.claude/memory/memory-index.json`, parse the JSON.
   ...
   4. Use the Write tool to write `{PROJECT_DIR}/.claude/memory/memory-index.json` back (preserve ALL other fields, 2-space indentation).

**Fallback Step 6**: Clean up temp file:
   1. Use the Read tool to read `{PROJECT_DIR}/.claude/memory/memory-index.json`, parse the JSON.
   2. Verify `memoryAppendedInThisRun` is `true`. If not → STOP (don't clean up).
   3. Use the Write tool to write `{PROJECT_DIR}/.claude/memory/delta_temp.txt` with empty content `""`.
   4. In the index JSON: set `deltaReady` to `false`, remove `memoryAppendedInThisRun`, remove `deltaCreatedAtMemoryMtime`.
   5. Use the Write tool to write `{PROJECT_DIR}/.claude/memory/memory-index.json` back (preserve ALL other fields, 2-space indentation).
