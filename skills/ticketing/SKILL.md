...
13. **Regressing state update:** If `.claude/memory/regressing-state.json` exists and is active, update it after ticket creation: append this ticket's ID to `"ticketIds"` array and update `"lastUpdatedAt"`. Phase transition is handled automatically by the PostToolUse hook. Only applies when regressing-state.json exists — standalone ticketing usage is unaffected.
